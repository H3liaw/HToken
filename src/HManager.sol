...
contract KAMManager is OwnableRoles, UUPSUpgradeable, Initializable, ReentrancyGuard, EIP712 {
    ...
    function initialize(address initialOwner, address _kusdToken, address _kusdVault) external initializer {
        ...
        validateExpiry(order.expiry)
    {
        ...
        IhUSDToken(kusdToken).mint(order.beneficiary, order.kusdAmount);
        ...
        validateExpiry(order.expiry)
    {
        ...
        IhUSDToken(kusdToken).burn(msg.sender, order.kusdAmount);
    ...
    function emergencyWithdraw(address asset, uint256 amount) external onlyRoles(EMERGENCY_ROLE) {
